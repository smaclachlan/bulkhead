...
**Resolving "exact Nix packaging approach" (previously open):** default to
`pkgs.dockerTools.buildLayeredImage` per container for phase 1. Layered images
share a base layer across the four images (smaller total pull/build cost) and
avoid hand-written Dockerfiles drifting from the flake. Fall back to a
Nix-wrapped plain Dockerfile *only* for a container where `dockerTools` proves
awkward (e.g. if the Workspace image needs a broad enough package set that
`buildImage`'s single-derivation model becomes unwieldy) — decide per-container
at build time, not up front. This is a reversible per-image choice, not a
cross-cutting one.
