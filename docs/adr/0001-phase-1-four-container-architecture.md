...
Pandora's goal (README.md) is a sandbox that keeps an LLM-driven agent contained:
...
- **Kubernetes instead of Compose for phase 1 supervision.** Rejected for now
  ...
  milestone; revisit if Pandora needs multi-host or multi-tenant scheduling.
