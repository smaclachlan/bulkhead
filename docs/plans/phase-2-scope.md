...
- **Mechanism**: `containerd` + `containerd-shim-kata-v2`, with Docker
  ...
  Bulkhead itself ever runs inside a VM (cloud dev box, CI), nested
...
- Proposed: `chat-mcp/src/chat_mcp/cli.py`, a small Python script (stdlib
  ...
  - One-shot: `bulkhead-chat send "message" --wait` for scripting/piping.
...
- **It's a fifth container** and Bulkhead's first Node-based one (the other
...
- CLI client: ship inside `chat-mcp/` (simplest - it's a client of that
  ...
  grows scope later (e.g. talking to multiple Bulkhead instances).
- Memory MCP's volume/backup story - a knowledge graph that's useful is
  also the first piece of Bulkhead state worth *not* losing on a container
