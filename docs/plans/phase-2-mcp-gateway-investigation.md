...
## What a gateway would actually buy Bulkhead
...
1. **Unified management** (fleet of MCP servers, SSO, secrets, audit UI) -
   not a Bulkhead problem yet. Two internal-only servers (workspace-mcp,
   memory-mcp) plus chat-mcp is not fleet scale, and Bulkhead's containment
...
**Mismatch with Bulkhead's own shape**: ToolHive wants to *own* the
container-orchestration layer for MCP servers - that's exactly what
Bulkhead's Nix-per-image + Compose-network-segmentation approach
...
ToolHive wholesale would mean either running Bulkhead's MCP servers *inside*
...
servers (workable, but then it's providing exactly one property Bulkhead
...
proposition doesn't match Bulkhead's current scale or its Nix/Compose-first
philosophy, and pulling in a full gateway just for tool-surface allowlisting
is disproportionate. Revisit if/when Bulkhead's own MCP-server count or
...
- Re-evaluate ToolHive (or similar) specifically for its Kubernetes
  operator story if Bulkhead ever needs multi-host/fleet deployment -
