...
netmode="$(docker inspect pandora-workspace --format '{{.HostConfig.NetworkMode}}' 2>/dev/null)"
if [ "$netmode" = "none" ]; then
  record step2.workspace-no-network pass "pandora-workspace NetworkMode is 'none'"
else
  record step2.workspace-no-network fail "pandora-workspace NetworkMode check" "got '$netmode'"
fi
...
if [ -z "${token:-}" ]; then
  ...
  run_id="$(date +%s)"
  marker="pandora-validate-${run_id}"
  fname="${marker}.txt"
...
" 2>/dev/null)"
  ...
  in_container="$(docker exec pandora-workspace sh -c "cat ${fname}" 2>&1)"
  if [ "$in_container" = "$content" ]; then
    record step5.file-in-container pass "file independently re-read inside pandora-workspace" "contents: $in_container"
  else
    record step5.file-in-container fail "file independently re-read inside pandora-workspace" "got: $in_container"
  fi
...
write_mcp_client() {
  docker compose exec -T "$1" python3 -c "import sys; open('/tmp/pandora_mcp_check.py','w').write(sys.stdin.read())" <<PYEOF
$mcp_client_py
PYEOF
}
...
run_mcp_check() {
  ...
  docker compose exec -T "$1" python3 /tmp/pandora_mcp_check.py "$2" "$3" "$4" 2>&1
}
...
marker7="pandora-mcp-check-$(date +%s)"
...
marker7b="pandora-mcp-escape-$(date +%s)"
